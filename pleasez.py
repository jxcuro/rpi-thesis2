# CODE 3.0.22 - AI Metal Classifier GUI with Gated Automation
...
# Version: 3.0.22 - MODIFIED: Removed temporal filtering and drift threshold from magnetism
#              -           processing to use values closer to the raw sensor output.
#              - MODIFIED: Classification now takes a fresh, stable sensor reading
#              -           instead of relying on the last displayed GUI value.
#              - NEW: Added a modifiable input hierarchy for the three inputs (Camera, Magnetism, LDC).
...
# --- NEW: Input Hierarchy State ---
INPUT_HIERARCHY = {
    "camera": "REQUIRED",
    "magnetism": "REQUIRED",
    "ldc": "REQUIRED"
}
HIERARCHY_OPTIONS = ["REQUIRED", "SECONDARY", "IGNORED"]
...
def preprocess_input(image_pil, mag_mT, ldc_rp_raw):
    ...
    # Check for required inputs based on the hierarchy
    if INPUT_HIERARCHY["camera"] == "REQUIRED" and image_pil is None:
        return None, "Camera input is REQUIRED but not available."
    if INPUT_HIERARCHY["magnetism"] == "REQUIRED" and mag_mT is None:
        return None, "Magnetism input is REQUIRED but not available."
    if INPUT_HIERARCHY["ldc"] == "REQUIRED" and ldc_rp_raw is None:
        return None, "LDC input is REQUIRED but not available."

    if interpreter is None or input_details is None or numerical_scaler is None:
        return None, "AI Model/Scaler not initialized."
    
    # Preprocess image input based on hierarchy
    if INPUT_HIERARCHY["camera"] != "IGNORED":
        try:
        ...
        # Create a blank image if ignored, as the model requires 2 inputs
        image_input = np.zeros((1, AI_IMG_HEIGHT, AI_IMG_WIDTH, 3), dtype=np.float32)
        print("Image input is IGNORED, using a blank placeholder.")

    # Preprocess numerical inputs based on hierarchy
    mag_val_for_model = float(mag_mT) if mag_mT is not None and INPUT_HIERARCHY["magnetism"] != "IGNORED" else 0.0
    ldc_val_for_model = float(ldc_rp_raw) if ldc_rp_raw is not None and INPUT_HIERARCHY["ldc"] != "IGNORED" else 0.0
    
    numerical_features = np.array([[mag_val_for_model, ldc_val_for_model]], dtype=np.float32)
    ...
    try:
        ...
        return None, f"Scaling numerical features failed: {e}"
    ...
    for detail in input_details:
        ...
        if len(shape) == 4 and shape[1] == AI_IMG_HEIGHT and shape[2] == AI_IMG_WIDTH:
            ...
            if shape[1] != scaled_numerical_features.shape[1]:
                return None, f"Model expects {shape[1]} numerical feats, got {scaled_numerical_features.shape[1]}."
    if image_input_index == -1 or numerical_input_index == -1:
        return None, "Failed to identify image/numerical input tensors."
...
def capture_and_classify():
    ...
    global save_output_var, INPUT_HIERARCHY
    global g_accepting_triggers
    ...
    # Capture image based on hierarchy setting
    img_captured_pil = None
    if INPUT_HIERARCHY["camera"] != "IGNORED":
        if not camera or not camera.isOpened():
    ...
    # Capture sensor readings based on hierarchy
    current_mag_mT = None
    if INPUT_HIERARCHY["magnetism"] != "IGNORED":
    ...
    current_rp_raw = None
    if INPUT_HIERARCHY["ldc"] != "IGNORED":
    ...
    if current_rp_raw is not None:
        current_rp_int = int(round(current_rp_raw))
    ...
    model_inputs, error_msg = preprocess_input(img_captured_pil, current_mag_mT, current_rp_raw)
    
    if model_inputs is None: 
        messagebox.showerror("AI Error", f"Data preprocessing failed.\n\nReason: {error_msg}")
        print(f"ERROR: Preprocessing abort. Reason: {error_msg}"); show_live_view(); return
    ...
    if save_output_var and save_output_var.get() == 1 and img_captured_pil is not None:
    ...
    if rv_image_label:
        if img_captured_pil:
            ...
            if placeholder_img_tk: rv_image_label.config(image=placeholder_img_tk, text="Img N/A"); rv_image_label.img_tk = placeholder_img_tk
            else: rv_image_label.config(image='', text="Img N/A"); rv_image_label.img_tk = None
...
def update_hierarchy():
    """Reads the comboboxes and updates the global INPUT_HIERARCHY dictionary."""
    global INPUT_HIERARCHY
    INPUT_HIERARCHY["magnetism"] = magnetism_combobox.get()
    INPUT_HIERARCHY["ldc"] = ldc_combobox.get()
    print(f"Input Hierarchy Updated: {INPUT_HIERARCHY}")
    messagebox.showinfo("Hierarchy Set", "Input hierarchy updated successfully.")
...
def setup_gui():
    ...
    global save_output_var, INPUT_HIERARCHY, HIERARCHY_OPTIONS
    global magnetism_combobox, ldc_combobox
    ...
    lv_hierarchy_frame = ttk.Labelframe(lv_controls_frame, text=" Input Hierarchy ", padding="8 4 8 8")
    lv_hierarchy_frame.grid(row=2, column=0, sticky="new", pady=(0,10)); lv_hierarchy_frame.columnconfigure(1, weight=1)

    ttk.Label(lv_hierarchy_frame, text="Magnetism:").grid(row=0, column=0, sticky="w", padx=(0, 8))
    magnetism_combobox = ttk.Combobox(lv_hierarchy_frame, values=HIERARCHY_OPTIONS, state="readonly", width=10)
    magnetism_combobox.set(INPUT_HIERARCHY["magnetism"])
    magnetism_combobox.grid(row=0, column=1, sticky="ew")

    ttk.Label(lv_hierarchy_frame, text="LDC:").grid(row=1, column=0, sticky="w", padx=(0, 8))
    ldc_combobox = ttk.Combobox(lv_hierarchy_frame, values=HIERARCHY_OPTIONS, state="readonly", width=10)
    ldc_combobox.set(INPUT_HIERARCHY["ldc"])
    ldc_combobox.grid(row=1, column=1, sticky="ew")

    ttk.Button(lv_hierarchy_frame, text="Set Hierarchy", command=update_hierarchy).grid(row=2, column=0, columnspan=2, pady=(10,0))
